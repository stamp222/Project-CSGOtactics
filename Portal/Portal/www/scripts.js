...
$(document).ready(function(){
    ...
    $("#canvas2").on("click", function(event){
        ...
        if(actualTask === "HE" || actualTask === "Flash" || actualTask === "Smoke"){
            ...
            switch (actualTask){
                case "HE":
                    base_image.src = '/www/icons/he.png';//img/base.png
                    console.log("granat" , event.clientX, event.clientY, canvas.height, canvas.width);
                    break;
                case "Flash":
                    base_image.src = '/www/icons/flash.png';//img/base.png
                    console.log("granat" , event.clientX, event.clientY);
                    break;

                case "Smoke":
                    base_image.src = '/www/icons/smoke.png';//img/base.png
                    console.log("granat" , event.clientX, event.clientY);
                    break;
            }
    ...
    $("#check").change(function(event){
        if($("#check option:selected").val() === "dust2"){
            $("#canvas2").css("background-image", 'url("/www/maps/dust2.jpg")');
        }
        else if($("#check option:selected").val() === "inferno"){
            $("#canvas2").css("background-image", 'url("/www/maps/inferno.jpg")');
        }
        else if($("#check option:selected").val() === "mirage"){
            $("#canvas2").css("background-image", 'url(/"www/maps/mirage.jpg")');
        }
        else if($("#check option:selected").val() === "overpass"){
            $("#canvas2").css("background-image", 'url("/www/maps/overpass.jpg")');
        }
        else if($("#check option:selected").val() === "train"){
            $("#canvas2").css("background-image", 'url("/www/maps/train.jpg")');
        }
        else if($("#check option:selected").val() === "cache"){
            $("#canvas2").css("background-image", 'url("/www/maps/cache.jpg")');
        }
